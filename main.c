        ...
        for (j=i+1;j<n;j++)
        {
            ...
            if (l<*(a+i*m+7)+*(a+j*m+7))
            {
                ...
                if (dot(v1vf,s)<0)
                {
                    ...
                    mul(v1v,v11,*(a+i*m+6),3);
                    mul(v2v,v21,*(a+j*m+6),3);
                    plu(v1vf,v11,v1vf,3);
                    plu(v1vf,v21,v1vf,3);
                    ...
                    plu(v2vf,v2,v2vf,3);
/*momentum correction
                    v1vf[0]/=((v1vf[0]**(a+i*m+6)+v2vf[0]**(a+j*m+6))/ini_momentum[0]);
                    v1vf[1]/=((v1vf[1]**(a+i*m+6)+v2vf[1]**(a+j*m+6))/ini_momentum[1]);
                    v1vf[2]/=((v1vf[2]**(a+i*m+6)+v2vf[2]**(a+j*m+6))/ini_momentum[2]);
                    v2vf[0]/=((v1vf[0]**(a+i*m+6)+v2vf[0]**(a+j*m+6))/ini_momentum[0]);
                    v2vf[1]/=((v1vf[1]**(a+i*m+6)+v2vf[1]**(a+j*m+6))/ini_momentum[1]);
                    v2vf[2]/=((v1vf[2]**(a+i*m+6)+v2vf[2]**(a+j*m+6))/ini_momentum[2]);
*/
...
void DP(double *a,double *res, int n, int m,double G,double time)
{
    ...
    for(j = 0; j < n; j++)
    {
        ...
        if (j != i && lenth(r,3)>*(a+i*m+7)+*(a+j*m+7))
        {
            mass = *(a+j*m+6);
            minus(l,r1,s,3);
            mul(s,re,G*mass/pow(lenth(s,3),3),3);
            plu(m1,re,m1,3);
        }
    }
    double k2[3];
